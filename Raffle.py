...
def show_raffle_100(user_id):
    ...
    btn = InlineKeyboardButton(text=f"{loc(Lk.participate, user_id)}: {raffle_cost} {loc(Lk.tickets, user_id)}", callback_data='raffle100')
    kb.add(btn)
    total_participants = 100
    current_participants = DataBase.get_raffle100_p_count()
    text = (loc(Lk.participants, user_id) +
            f'{loc(Lk.participants, user_id)}: {current_participants}/{total_participants}')
